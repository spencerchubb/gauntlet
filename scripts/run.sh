...
if [ "$version" == "dev" ]; then
    ...
    nohup uvicorn main:app --host 0.0.0.0 --port 443 --ssl-keyfile=/etc/letsencrypt/live/pandapal.app/privkey.pem --ssl-certfile=/etc/letsencrypt/live/pandapal.app/fullchain.pem > log.txt 2>&1 &
else
